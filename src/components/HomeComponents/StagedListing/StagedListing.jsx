    ...
    <div className="vs-container">
      ...
      <div id="recent-listings">
        <OwlCarousel className="owl-theme" {...customOptions}>
          {listings.map((item) => (
            <div key={item.id} class="aa">
              {item.images.map((image, index) => (
                <div id="item">
                  <img
                    key={`${item.id}-${index}`}
                    src={listing(image)}
                    className="listinig-img"
                    alt=""
                  />
                </div>
              ))}
      ...
      {/* {console.log(listings)}
        {listings.map((el) => {
          return (
            <div key={el.id}>
              <img
                src={IMG(el.images[0])}
                alt="not working dunno"
                style={{ width: "400px", height: "500px" }}
              />

              <p>{el.agent}</p>
              {el.images.map((image, imgIndex) => {
                return (
                  <div>
                    <img key={imgIndex} src={IMG(image)} alt="not working" />
                  </div>
                );
              })}
            </div>
        ...
        <img src={imageOf} alt="not working" /> */}
    </div>
  );
};
